from pettingzoo.mpe import simple_v3, simple_push_v3
...
class Learner(): 
    ...
    def train(self):
        ...
        for episode in tqdm(range(self.episodes)):
            ...
            eps_rewards = {key: 0 for key in self.env.agents}
            for cur_step in range(0, 25):
                ...
                total_reward = sum(value for value in rewards.values())
                for agent in self.env.agents: 
                    self.buffer[agent].append((observations[agent], actions[agent], total_reward, next_observations[agent], terminations[agent]))
                    eps_rewards[agent] += rewards[agent]
                    if len(self.buffer[agent]) > self.batchsize:
                        self.updateModel(self.models[agent], self.optimizers[agent], self.criterion, self.gamma, self.buffer[agent], self.batchsize)
    ...
    def atr(self): 
        return average_total_reward(parallel_to_aec(self.env), max_episodes=self.episodes, max_steps=self.episodes*25)
...
class TagLearner_A2C(): 
    def __init__(self, learning_rate_actor = 1e-3, learning_rate_critic = 1e-3, gamma = 0.99, episodes = 1000): 
        ...
        self.env = simple_v3.env(render_mode='human')
        self.env.reset()
        self.buffer = {key: [] for key in self.env.agents}
        for agent in self.env.agents:
            output_actions = self.env.action_space(agent).n
...
if __name__ == '__main__':
    num_episodes = 1000
    tag = Learner(episodes = num_episodes)
    rewards = tag.train()
    out_reward = 0
